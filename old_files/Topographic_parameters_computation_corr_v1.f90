...
!TEI_pointers_array
  SUBROUTINE computing_WL_exposure_indexes(TEI_pointers_array, TEI_drying_effect_correction, &
    ...
    !The following loops are activated onmy if the user wants to activate the
...
!PRINT*, "MINVAL"
!PRINT*, MINVAL(TEI_pointers_array(1)%tei_arr_ptr), MINVAL(TEI_drying_effect_correction(1)%tei_arr_ptr)
!PRINT*, "MAXVAL"
!PRINT*, MAXVAL(TEI_pointers_array(1)%tei_arr_ptr), MAXVAL(TEI_drying_effect_correction(1)%tei_arr_ptr)

!do m=1, nbr_wdir 
!   TEI_drying_effect_correction(m)%tei_arr_ptr=TEI_drying_effect_correction(m)%tei_arr_ptr/100
!end do
