...
CONTAINS
  ...
  SUBROUTINE computing_wind_directions(locvar__sorted_wind_directions_array)

    IMPLICIT NONE

    DOUBLE PRECISION, DIMENSION(:,:,:), ALLOCATABLE, INTENT(INOUT) :: locvar__sorted_wind_directions_array

   ALLOCATE(locvar__sorted_wind_directions_array(1:lr_climate_data_x_size, 1:lr_climate_data_y_size, 1:t_extent))

    locvar__sorted_wind_directions_array(:,:,:) = 0
          ...
          DO i=1, hr_topo_x_size
             ...
             DO m=1, nbr_wdir
                IF ((wind_direction .GE. wdir_angle_boundaries(m)) & 
                   ...
                   locvar__sorted_wind_directions_array(i,j,t) = m
                END IF
             END DO
          END DO
       END DO
